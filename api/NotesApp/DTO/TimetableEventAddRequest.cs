...
public class TimetableEventAddRequest
{
    ...
    [RegularExpression(@"^[1-5]\.(0?[1-9]|[1-9]\d|100)$", ErrorMessage = "Lecture Room must be in the format X.Y, where X is between 1 and 5, and Y is between 1 and 100")]
    [StringLength(10)]
    public string? EventRoom { get; set; }
    
    [Required(ErrorMessage = "Type can't be empty")]
    [RegularExpression(@"^(practise|lecture)$", ErrorMessage = "Type must be either 'practise' or 'lecture'")]
    [StringLength(10)]
